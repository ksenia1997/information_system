...
if(isset($_POST["added"])){
  ...
  if(empty($rodne_cislo) || (empty($meno)) || (empty($priezvisko))
    ...
    if($count > 0){
      ...
      $query1 = $db->query('SELECT * FROM Personal');
      $max_id_sestry = 0;
      $max_id_veterinara = 0;
      while($r = $query1->fetch()) {
        if($r['ID_Veterinara'] > $max_id_veterinara) {
          $max_id_veterinara = $r['ID_Veterinara'];
        }
      }
      $max_id_veterinara += 1;
      $id_sestry = null;
      ...
      $query = $db->prepare($sql);
      $query->execute(array($_POST["Rodne_cislo"], $_POST["Meno"], $_POST["Priezvisko"], $_POST["Titul"], $_POST["Adresa"], $_POST["Cislo_uctu"], $_POST["Hodinova_mzda"], 'veterinar', $id_sestry, $max_id_veterinara));
      $message = "";
      $rodne_cislo = "";
      $meno = "";
      $priezvisko = "";
