...
if(isset($_POST["added"])){
  ...
  $veterinar_id = $_POST["ID_Veterinara"];
  $zviera_id = $_SESSION["id1"];
...
<div class="container" style="width:500px;">
    ...
    <form method="post">
        ...
        <select name = "ID_Veterinara" class="custom-select mb-3">
          <option value="">--- Vybrať ---</option>
          <?php
            include 'database/connect.php';
            $sql = "SELECT * FROM Personal WHERE Typ = ?";
            $query = $db->prepare($sql);
            $query->execute(array("veterinar"));
            while($r = $query->fetch(PDO::FETCH_OBJ)) {
          ?>
          <option value="<? echo $r->ID_Veterinara; ?>">
              <?php
                echo $r->Meno;
                echo" ";
                echo $r->Priezvisko;
