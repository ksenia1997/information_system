...
if(isset($_POST["delete"])){
    ...
    $sql = "DELETE FROM Personal WHERE ID_Sestry = ?";
...
if(isset($_POST["edit"])or $_POST["added"]){
    if(isset($_POST["edit"])){
        //hladanie v databazi
        include 'database/connect.php';
        $sql = "SELECT * FROM Personal WHERE ID_Sestry = ? ";
        $query = $db->prepare($sql);
            ...
            if($_SESSION["role"] != "admin"){
                ...
                if(isset($_POST["added"])){
                    ...
                    if(empty($rodne_cislo) || (empty($meno)) || (empty($priezvisko))
                        ...
                        $sql = "UPDATE Personal SET Rodne_cislo = ?, Meno = ?, Priezvisko = ?, Titul = ?, Adresa = ?, Cislo_uctu = ?, Hodinova_mzda = ? WHERE ID_Sestry = ? ";
                        $query = $db->prepare($sql);
...
<div class="container" style="width:500px;">
    ...
    <form method="post">
        ...
        <br />
        <button type="submit"class="btn btn-outline-danger" Name = "added" Value="<?php echo $id_sestry?>">Upraviť</button>
    </form>
